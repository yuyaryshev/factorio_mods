require "util"
local chance_setting = 1
local max_inf_count = 1
local infinity_in_start_area = true
...
function init()
	...
	global.inf_resource_chance.default = 1
...
script.on_event(defines.events.on_resource_depleted, function(event)
	...
	if global.resources[name] then
		...
		if new_field then
			...
			local inf_count = 0
			if math.random() < chance and (infinity_in_start_area or util.distance(field.position,{x = 0, y = 0}) > global.start_area[surface.name]) then
				for _,ore in pairs(field.ores) do
					local ore_value = 0
					local dist = util.distance(ore.position,field.position)
					if max_inf_count == 0 then
						...
						if inf_count < max_inf_count and dist < 2*max_inf_count then
							max_inf_count = 0
							ore_value = global.resource_amount[ore.name][ore.surface.name]*3/2
