...
local one_item_type_per_train = use_filters_on_train
...
function ProcessRequest(request)
  ...
  for item, count in pairs (request.itemlist) do
    ...
    for i=1, #orders do
	  local type_matches = (orders[i].loadingList[1].name == loadingList.name and #orders[i].loadingList == 1)
	  if orders[i].fromID == fromID and itype == "item" and orders[i].loadingList[1].type == "item" and (not one_item_type_per_train or type_matches) then
	    if type_matches then
          orders[i].loadingList[1].count = orders[i].loadingList[1].count + loadingList.count
          orders[i].loadingList[1].stacks = orders[i].loadingList[1].stacks + loadingList.stacks
		else
  ...
  for orderIndex=1, #orders do
	...
	-- set filters on train, only if one item type and option is "ON"
	if use_filters_on_train then
	  for j=1,#selectedTrain.cargo_wagons do
	    local cargo_wagon_inventory = selectedTrain.cargo_wagons[j].get_inventory(defines.inventory.cargo_wagon)
		if cargo_wagon_inventory.supports_filters() then
		  local filter_value = loadingList[1].name
		  local size = #cargo_wagon_inventory
          if #loadingList == 1 then
	        for i = 1, size do
	          cargo_wagon_inventory.set_filter(i, filter_value)
	        end
          else
            for i = 1, size do
	          cargo_wagon_inventory.set_filter(i, nil)
	        end	  
          end
		end
      end
