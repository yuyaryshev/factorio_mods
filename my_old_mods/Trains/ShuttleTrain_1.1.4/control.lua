...
function init()
	...
	if global.shuttleTrains == nil then
		global.shuttleTrains = game.surfaces[1].find_entities_filtered{area = {{-10000,-10000}, {10000,10000}}, name="shuttleTrain"} or {}
		local tmp_trains = game.surfaces[1].find_entities_filtered{area = {{-10000,-10000}, {10000,10000}}, name="atomic-shuttle"} or {}
		for _,v in pairs(tmp_trains) do table.insert(global.shuttleTrains[k], v) end
	end
    load()
end
...
function isShuttle(locomotive)
    if locomotive.name == "shuttleTrain" or locomotive.name == "atomic-shuttle" then
        return true
    else
        return false
    end
